...
- Wording, settled and closed: the owner extended BUG-004's ruling three times on 2026-09-23 — `P(和牌)` -> `P(胡牌)`, then `和牌值` -> `胡牌值`, then every remaining `和牌`. No winning-sense 和 is left in the tracked tree: 11 `和牌` strings in the UI and four docs, plus three that mean a win without the compound (`含和的那張`, `存活P(和)` twice, `快和的`). The bare character 和 is untouched where it means "and". The two `和牌` mentions left in this report describe what the delegated round shipped, so changing them would falsify the history. `test_ui_spells_a_win_the_taiwanese_way` keeps the compound out of `server/static/js/`, matching the precedent `b6e7952` set when it replaced 榮和 with 胡牌.
